...
/**
 * Only used for Debezium JSON format. Tool class used to convert from {@link JsonNode} to {@link
 * SeaTunnelRow}. *
 */
class JsonToRowConverters implements Serializable {
    ...
    private double convertToDouble(JsonNode jsonNode) {
        if (jsonNode.isDouble()) {
            ...
            JsonNode scale = jsonNode.get("scale");
            if (scale != null) {
                // compatible with debezium json format
                return new BigDecimal(
                                new BigInteger(jsonNode.get("value").binaryValue()), scale.asInt())
                        .floatValue();
            }
