...
public class SqliteURLParser {
    private static final Pattern SQLITE_URL_PATTERN =
            Pattern.compile("^(?<url>jdbc:sqlite:(//)?)(?<database>.+?)((?<suffix>\\?.*)?)$");
    ...
    public static JdbcUrlUtil.UrlInfo parse(String url) {
        ...
        if (matcher.find()) {
            ...
            return new JdbcUrlUtil.UrlInfo(url, urlWithoutDatabase, null, null, database, suffix);
        }
