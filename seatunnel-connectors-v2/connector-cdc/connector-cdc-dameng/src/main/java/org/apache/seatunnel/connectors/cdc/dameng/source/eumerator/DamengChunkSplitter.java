...
@Slf4j
@AllArgsConstructor
public class DamengChunkSplitter implements JdbcSourceChunkSplitter {
    ...
    private List<ChunkRange> splitUnevenlySizedChunks(
        ...
        while (chunkEnd != null && ObjectUtils.compare(chunkEnd, max) <= 0) {
            ...
            splits.add(ChunkRange.of(chunkStart, chunkEnd));
            // may sleep a while to avoid DDOS on Database server
            if (count++ % 10 == 0) {
                try {
                    Thread.sleep(100);
                ...
                log.info("DamengChunkSplitter has split {} chunks for table {}", count, tableId);
            }
