...
public interface DamengStreamingChangeEventSourceMetricsMXBean
    ...
    /**
     * fetches counter of redo switches for the last day. If this number is high , like once in 3
     * minutes, the troubleshooting on the database level is required.
     *
     * @return counter
     */
    ...
    /**
     * LogMiner view query returns number of captured DML , Commit and Rollback. This is what we
     * call a batch.
     *
    ...
    /**
     * Maximum number of entries in LogMiner view to fetch. This is used to set the diapason of the
     * SCN in mining query. If difference between "start SCN" and "end SCN" to mine exceeds this
     * limit, end SCN will be set to "start SCN" + batchSize
     *
     * @return the limit
     */
    int getBatchSize();
    ...
    /**
     * this gives ability to manipulate number of entries in LogMiner view to fetch. It has limits
     * to prevent abnormal values
     *
     * @param size limit
     */
    ...
    /**
     * @return number of milliseconds for connector to sleep before fetching another batch from the
     *     LogMiner view
     */
    long getMillisecondToSleepBetweenMiningQuery();

    /**
     * sets number of milliseconds for connector to sleep before fetching another batch from the
     * LogMiner view
     *
    ...
    /**
     * @return the minimum time in milliseconds spent processing results from a single LogMiner
     *     session
     */
    long getMinBatchProcessingTimeInMilliseconds();

    /**
     * @return the maximum time in milliseconds spent processing results from a single LogMiner
     *     session
     */
    ...
    /**
     * Lag can temporarily be inaccurate on DST changes. This is because the timestamps received
     * from LogMiner are in the database local time and do not contain time zone information.
     *
     * @return lag in milliseconds of latest captured change timestamp from transaction logs and
     *     it's placement in the transaction buffer.
     */
    ...
    /**
     * @return total duration in milliseconds the last commit operation took in the transaction
     *     buffer
     */
    long getLastCommitDurationInMilliseconds();

    /**
     * @return the duration in milliseconds that the longest commit operation took in the
     *     transaction buffer
     */
