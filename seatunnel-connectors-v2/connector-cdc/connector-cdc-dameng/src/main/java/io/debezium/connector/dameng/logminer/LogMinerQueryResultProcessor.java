...
import io.debezium.connector.dameng.logminer.parser.SelectLobParser;
...
class LogMinerQueryResultProcessor {
    ...
    void processResult(ResultSet resultSet) throws SQLException {
        ...
        while (context.isRunning() && hasNext(resultSet)) {
            ...
            LOGGER.trace(
                    "scn={}, operationCode={}, operation={}, table={}, segOwner={}, userName={}, rowId={}, rollbackFlag={}",
                    scn,
                    operationCode,
                    operation,
                    tableName,
            ...
            String logMessage =
                    String.format(
                            "transactionId=%s, SCN=%s, table_name=%s, segOwner=%s, operationCode=%s, offsetSCN=%s, "
                                    + " commitOffsetSCN=%s",
                            txId,
                            scn,
                            tableName,
                ...
                case RowMapper.SELECT_LOB_LOCATOR:
                    {
                        if (!connectorConfig.isLobEnabled()) {
                            LOGGER.trace(
                                    "SEL_LOB_LOCATOR operation skipped for '{}', LOB not enabled.",
                                    redoSql);
                            continue;
                        }
                        LOGGER.trace("SEL_LOB_LOCATOR: {}, REDO_SQL: {}", logMessage, redoSql);
                        final TableId tableId =
                                RowMapper.getTableId(connectorConfig.getCatalogName(), resultSet);
                        final Table table = schema.tableFor(tableId);
                        if (table == null) {
                            LogMinerHelper.logWarn(
                                    streamingMetrics,
                                    "SEL_LOB_LOCATOR for table '{}' is not known to the connector, skipped.",
                                    tableId);
                            continue;
                        }
                        transactionalBuffer.registerSelectLobOperation(
                                operationCode,
                                txId,
                                scn,
                                tableId,
                ...
                case RowMapper.LOB_WRITE:
                    {
                        ...
                        if (schema.tableFor(tableId) == null) {
                            LogMinerHelper.logWarn(
                                    streamingMetrics,
                                    "LOB_WRITE for table '{}' is not known to the connector, skipped.",
                                    tableId);
                            continue;
                        }
                        transactionalBuffer.registerLobWriteOperation(
                                operationCode,
                                txId,
                                scn,
                                tableId,
                ...
                case RowMapper.LOB_ERASE:
                    {
                        ...
                        if (schema.tableFor(tableId) == null) {
                            LogMinerHelper.logWarn(
                                    streamingMetrics,
                                    "LOB_ERASE for table '{}' is not known to the connector, skipped.",
                                    tableId);
                            continue;
                        }
                        transactionalBuffer.registerLobEraseOperation(
                                operationCode,
                                txId,
                                scn,
                                tableId,
                ...
                case RowMapper.DELETE:
                    {
                        ...
                        if (redoSql != null) {
                            ...
                            if (rollbackFlag == 1) {
                                // DML operation is to undo partial or all operations as a result of
                                // a rollback.
                                // This can be situations where an insert or update causes a
                                // constraint violation
                                // and a subsequent operation is written to the logs to revert the
                                // change.
                                transactionalBuffer.undoDmlOperation(txId, rowId, tableId);
                                continue;
                            }

                            transactionalBuffer.registerDmlOperation(
                                    operationCode,
                                    txId,
                                    scn,
                                    tableId,
        ...
        LOGGER.debug(
                "{} Rows, {} DMLs, {} Commits, {} Rollbacks, {} Inserts, {} Updates, {} Deletes. Processed in {} millis. "
                ...
                deleteCounter,
                totalTime.toMillis(),
                streamingMetrics.getLagFromSourceInMilliseconds(),
                offsetContext.getScn(),
                offsetContext.getCommitScn(),
    ...
    private Table getTableForDmlEvent(TableId tableId) throws SQLException {
        ...
        if (table == null) {
            if (connectorConfig.getTableFilters().dataCollectionFilter().isIncluded(tableId)) {
                ...
                LogMinerHelper.logWarn(
                        streamingMetrics,
                        "DML for table '{}' that is not known to this connector, skipping.",
                        tableId);
            }
        }
        return table;
    ...
    private String getTableMetadataDdl(TableId tableId) throws SQLException {
        ...
        // LogMiner query results.
        // This should have negligible overhead as this should happen rarely.
        try (DamengConnection connection =
                new DamengConnection(
                        connectorConfig.getJdbcConfig(), () -> getClass().getClassLoader())) {
            return connection.getTableMetadataDdl(tableId);
        }
    }

    /**
     * This method is warning if a long running transaction is discovered and could be abandoned in
     * the future. The criteria is the offset SCN remains the same in 25 mining cycles
     */
    ...
    private LogMinerDmlEntry parse(String redoSql, Table table, String txId) {
        ...
        try {
            ...
            message.append(" Please open a Jira issue with the statement '")
                    .append(redoSql)
                    .append("'.");
        ...
        if (dmlEntry.getOldValues().length == 0) {
            if (RowMapper.UPDATE == dmlEntry.getOperation()
                    || RowMapper.DELETE == dmlEntry.getOperation()) {
