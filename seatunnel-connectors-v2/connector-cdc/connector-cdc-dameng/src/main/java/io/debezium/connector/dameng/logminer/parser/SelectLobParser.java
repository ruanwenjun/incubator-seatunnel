...
public class SelectLobParser {
            ...
            try {
                ...
                if (start != -1) {
                    ...
                    if (sql.indexOf(SELECT, start) == start) {
                        ...
                        if (sql.indexOf(BLOB_LOCATOR, start) == start
                                || sql.indexOf(BLOB_BUFFER, start) == start) {
                            binary = true;
                        }
                ...
                throw new ParsingException(
                        null, "Parsing failed for SEL_LOB_LOCATOR sql: '" + sql + "'", t);
            }
        }
    ...
    /**
     * @return true if the column being modified is a BLOB; otherwise false indicates a CLOB data
     *     type.
     */
        ...
        for (int i = index; i < sql.length(); ++i) {
            ...
            index =
                    parseColumnValue(
                            sql,
                            index,
                            LogMinerHelper.getColumnIndexByName(columnName.toString(), table));
            index += 1; // space
    ...
    private int parseOperator(String sql, int index) {
        ...
        for (int i = index; i < sql.length(); ++i) {
            ...
            if (!initialSpace && c == ' ') {
                ...
                throw new ParsingException(
                        null, "Failed to parse operator at index " + i + ": " + sql);
            }
        }
        return index;
    }
